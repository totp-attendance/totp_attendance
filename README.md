...
## 교수별 시간표 (과목) — 로그인 후 홈 화면
교수마다 본인 정규 수업을 등록하고 거기서 출석을 바로 시작. **로그인하면 홈(`/`)이 시간표**.
- `/`(=`/timetable`) — 본인 **주간 시간표 그리드**(월~금) + 수업 추가/수정/삭제 (과목명·요일·시작·종료·강의실).
- **오늘 수업** 박스 → **[출석 시작]** 1번 → 세션 자동 생성(이름=과목+날짜, 현장확인 QR 자동) → QR 화면.
- **중복 방지**: 같은 과목·같은 날 이미 시작했으면 그 세션으로 이동.

## 세션(출석) 관리
- `/sessions` — 연 출석 세션 목록 (소유 격리: 본인 것만, 관리자는 전체).
- 각 세션: **QR 화면**(교사용 회전 QR), **출석부**(명단·CSV), **열기/닫기**, **삭제**(세션+출석 기록 함께).
- 시간표 없이 일회성 수업은 `/sessions` 의 생성 폼으로 직접 생성 가능.
...
- 학생 관리 화면(`/students`)에 자가등록 링크/QR 표시 → 학생에게 공유.
- 한계: 외부인은 등록키로 차단되나, **같은 반 내 학번 선점 도용**은 막지 못함(완전 차단은 OAuth/이메일 인증 필요).

## 현장 확인 (QR, 항상 적용) — 원격 출석 차단
모든 세션에 자동 적용 (옵션 아님):
- 교사 QR 화면(`/teacher/<id>`)에 **QR** 표시 (기본 10초마다 갱신, HMAC 챌린지 내장 → 위조 불가).
