...
def _validate(session_id, s, sid_val, code, ip, challenge):
    ...
    if config.is_rate_limited(rl_key):
        return "시도가 너무 많습니다. 잠시 후 다시 시도해 주세요.", "err", None
    if not s["open"]:
        return "출석이 마감된 세션입니다.", "err", None
    # 현장 확인: 교실 화면의 QR 을 스캔해 얻은 유효 챌린지 필요 (원격 차단)
    if s["require_qr"] and not config.verify_challenge(session_id, challenge):
        return "교실 화면의 QR을 스캔해 주세요. (만료되면 다시 스캔)", "err", None
    if not sid_val or not code:
        return "학번과 코드를 모두 입력해 주세요.", "err", None
    if not student:
        return ("등록되지 않은 학번입니다. 교사에게 등록을 요청하세요.", "err", None)
